...
sceneJson = sceneJson
  ...
  // ⚠️ 横线与上半圆交接处的小过渡光弧：只在硬接点附近加"很小的光弧"连接，
  //    不是长贝塞尔。横线端点（±0.16 处，线尾渐隐前）到上半圆弧 20° 处，
  //    用半径很小的圆弧段（半圆弧）平滑过渡，把"平线→弯弧"的硬折角填平。
  .replace(
    'float beam = getBeam(uv, pos, 0.6000, 0.0000, 0.5000, uBeamThickness, uTime, 0.7300, uResolution);',
    'float beam = getBeam(uv, pos, 0.6000, 0.0000, 0.5000, uBeamThickness, uTime, 0.7300, uResolution);\\nfloat dx = uv.x - pos.x;\\nfloat horizon = exp(-dx * dx * 25.0) * exp(-abs(uv.y - pos.y) * 90.0) * 4.0;\\nbeam += horizon;\\nfloat hw = uBeamThickness * 0.35;\\nfloat rc = 0.045;\\nvec2 ctr = vec2(pos.x + 0.145, pos.y + rc);\\nvec2 ctl = vec2(pos.x - 0.145, pos.y + rc);\\nfloat ang = calculateAngle(uv, ctr);\\nfloat adr = abs(angularDifference(ang, 0.5 * PI));\\nfloat drc = abs(length(uv - ctr) - rc);\\nbeam += (1.0 - smoothstep(0.0, hw, drc)) * (1.0 - smoothstep(0.0, 0.7, adr)) * 3.0;\\nfloat adl = abs(angularDifference(calculateAngle(uv, ctl), 0.5 * PI));\\nfloat dlc = abs(length(uv - ctl) - rc);\\nbeam += (1.0 - smoothstep(0.0, hw, dlc)) * (1.0 - smoothstep(0.0, 0.7, adl)) * 3.0;',
  )
